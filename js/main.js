...
function applyTheme(theme) {
  ...
  toggleIcon.textContent = ICONS[theme];
  localStorage.setItem('ycb-theme', theme);
}

// On load — respect saved preference, else default dark
(function initTheme() {
  const saved = localStorage.getItem('ycb-theme');
  if (saved === 'light') {
    applyTheme('light');
  } else {
    applyTheme('dark');
...
themeToggle.addEventListener('click', () => {
  ...
  toggleIcon.style.transform = 'rotate(360deg) scale(1.3)';
  setTimeout(() => { toggleIcon.style.transform = ''; }, 500);
});
